...
export async function sbSaveUser(u) {
  const { error } = await supabase.from("users").upsert({
    phone: u.phone, name: u.name || null, village: u.village || null,
...
export async function sbSaveDriver(d) {
  const { error } = await supabase.from("drivers").upsert({
    phone: d.phone, name: d.name || null, village: d.village || null,
...
export async function sbSaveBooking(b) {
  const { error } = await supabase.from("bookings").upsert({
    id: b.id, customer_phone: b.ph, customer_name: b.name || null,
    village: b.village || null, service_id: b.serviceId || null,
    service_name_hi: b.serviceNameHi || null, icon: b.icon || null,
    ...
    created_at: b.at || new Date().toISOString(),
  }, { onConflict: "id" });
...
export async function sbSaveSubscription(phone, acres, amt) {
  const { error } = await supabase.from("subscriptions").insert({
    user_phone: phone, acres: parseFloat(acres), amount: amt,
...
export async function sbGetAllBookings() {
...
export async function sbSendNotification(message, send_to = "all", title = "") {
  const { error } = await supabase.from("notifications").insert({ message, send_to, title });
  if (error) console.error("sbSendNotif:", error.message);
}

export async function sbGetNotifications() {
  const { data } = await supabase.from("notifications").select("*").order("sent_at", { ascending: false }).limit(30);
  return data || [];
}
